...
class _TrackOrderState extends State<TrackOrder> {
  ...
  Future<void> printNameWithDifferentFonts(BuildContext context) async {
    final profile = await CapabilityProfile.load();
    final printer = NetworkPrinter(PaperSize.mm80, profile);

    // the app will connect to the printer (make sure to replace '192.168.1.100' with the actual printer's IP address)
    final posPrintResult = await printer.connect('192.168.1.100', port: 9100);

    if (posPrintResult != PosPrintResult.success) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Error connecting to the printer.'),
      ));
      return;
    }

    final textStyles = [
      PosStyles(align: PosAlign.center, height: PosTextSize.size1, width: PosTextSize.size1),
      PosStyles(align: PosAlign.center, height: PosTextSize.size2, width: PosTextSize.size2),
      PosStyles(align: PosAlign.center, height: PosTextSize.size3, width: PosTextSize.size3),
      PosStyles(align: PosAlign.center, height: PosTextSize.size4, width: PosTextSize.size4),
    ];

    final name = 'Your Name';

    for (final style in textStyles) {
      printer.text(name, styles: style);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text('Print Your Name', style: TextStyle(fontSize: 20)),
          SizedBox(height: 20),
          ElevatedButton(
            onPressed: () => printNameWithDifferentFonts(context),
